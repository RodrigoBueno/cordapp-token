...
class TransferCadastroFlowTest {
    ...
    @Test
    fun `deve transferir todos os dados do cadastro`() {
        ...
        listOf(a, b).forEach {
            it.transaction {
                ...
                it.services.vaultService.queryBy<CoinState>().states.single().state.data
                it.services.vaultService.queryBy<DocumentoState>().states.single().state.data
                it.services.vaultService.queryBy<AccountState>().states.single().state.data
            }
        }
    }
}