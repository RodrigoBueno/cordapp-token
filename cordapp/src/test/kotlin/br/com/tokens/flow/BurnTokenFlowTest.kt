...
class BurnTokenFlowTest {
    ...
    init {
        listOf(a, b).forEach {
            ...
            it.registerInitiatedFlow(BurnTokenFlow.Responder::class.java)
        }
    }
    ...
    @Test
    fun `deve consumir token`() {
        val future = a.startFlow(BurnTokenFlow.Initiator(accountState.state.data,
                tokenState.state.data.linearId.id,
                5))
        network.runNetwork()
        future.get()
            a.transaction {
                a.services.vaultService.queryBy<TokenState>().states.single().state.data
                a.services.vaultService.queryBy<CadastroPessoaState>().states.single().state.data
                assertEquals(1, a.services.vaultService.queryBy<CoinState>().states.size)
                assertEquals(5, a.services.vaultService.queryBy<CoinState>().states.single().state.data.value.value)
                a.services.vaultService.queryBy<DocumentoState>().states.single().state.data
                assertEquals(2, a.services.vaultService.queryBy<AccountState>().states.size)
            }
    }

    @Test
    fun `deve consumir todos os tokens`() {
        val future = a.startFlow(BurnTokenFlow.Initiator(accountState.state.data,
                tokenState.state.data.linearId.id,
                10))
        network.runNetwork()
        ...
        a.transaction {
            a.services.vaultService.queryBy<TokenState>().states.single().state.data
            a.services.vaultService.queryBy<CadastroPessoaState>().states.single().state.data
            assertEquals(0, a.services.vaultService.queryBy<CoinState>().states.size)
            a.services.vaultService.queryBy<DocumentoState>().states.single().state.data
            assertEquals(2, a.services.vaultService.queryBy<AccountState>().states.size)
        }
    }
