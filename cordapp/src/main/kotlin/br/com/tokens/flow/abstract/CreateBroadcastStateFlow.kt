...
abstract class CreateBroadcastStateFlow<T>(val valueToCreate: T) : FlowLogic<SignedTransaction>() {

    @Suspendable
    override fun call(): SignedTransaction {
        val notary = serviceHub.networkMapCache.notaryIdentities.first()
        val command = Command(BroadCastedStateContract.Commands.Create(), ourIdentity.owningKey)
    ...
    open fun <T, K : CommandData> buildTransactionBuilder(notary: Party, command: Command<K>, output: MultipleOwnersState<T>) =
            TransactionBuilder(notary)
                    .addCommand(command)
                    .addOutputState(output, BroadCastedStateContract::class.java.canonicalName)
