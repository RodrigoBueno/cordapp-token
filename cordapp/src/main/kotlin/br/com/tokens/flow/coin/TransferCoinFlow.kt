...
object TransferCoinFlow{

    @InitiatingFlow
    @StartableByRPC
    class Initiator(val from: AccountState, val to: AccountState, val tokenId: UUID, val value: Int): FlowLogic<SignedTransaction>() {
        ...
        override fun call(): SignedTransaction {
            ...
            val accountTokens = builder {
                ...
                val criteria2 = QueryCriteria.VaultCustomQueryCriteria(CoinSchemaV1.PersistentCoin::userExternalId.equal(from.value.externalId))
            ...
            val command = Command(CoinContract.Commands.Transfer(),
            ...
            if (troco) {
                ...
                val state2 = stateComTroco.copy(coin = stateComTroco.value.copy(value = diferenca, owner = to.value), linearId = UniqueIdentifier())
                txBuilder.addOutputState(state1, CoinContract::class.java.canonicalName)
                txBuilder.addOutputState(state2, CoinContract::class.java.canonicalName)
                novaLista.map { it.state.data }.forEach {
                    val state = it.copy(coin = it.value.copy(owner = to.value), linearId = UniqueIdentifier())
                ...
                selectedTokens.map { it.state.data }.forEach {
                    val state = it.copy(coin = it.value.copy(owner = to.value), linearId = UniqueIdentifier())
                    txBuilder.addOutputState(state, CoinContract::class.java.canonicalName)
                }
                selectedTokens.forEach {
